...
class DKT(nn.Module):
    ...
    def __init__(
        ...
        self.duplex_classifier = duplex_classifier
        ...
        if self.duplex_classifier:
            in_dim, out_dim = self._get_clf_dim()
    ...
    def _get_clf_dim(self):
        if self.duplex_classifier:
            in_dim = self.embed_dim
            out_dim = self.nb_classes_per_task[-1]
        else:
            raise NotImplementedError(f'Unknown classifier type')
    ...
    def param_groups(self):
        return {
            ...
            'old_heads': self.head[:-self.nb_classes_per_task[-1]].parameters() \
                if self.duplex_classifier else \
                self.head.parameters(),
            'new_head': self.head[-1].parameters() if self.duplex_classifier else self.head.parameters(),
