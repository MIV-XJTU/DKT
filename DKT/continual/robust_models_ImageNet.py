...
class Attention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        q = self.q(x[:, :196]).reshape(B, 196, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, 196, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class Block(nn.Module):
    ...
    def forward(self, x, tokens, mode):
        if self.use_CA:
            x_cls = x[:, :1]
...
class JointCA(nn.Module):
    ...
    def __init__(self, dim, num_heads=6, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., fc=nn.Linear):
        ...
        self.q1 = fc(dim, dim, bias=qkv_bias)
        self.k1 = fc(dim, dim, bias=qkv_bias)
        self.v1 = fc(dim, dim, bias=qkv_bias)
        self.attn_drop1 = nn.Dropout(attn_drop)
        self.proj1 = fc(dim, dim)
...
class conv_embedding(nn.Module):
    def __init__(self, in_channels, out_channels, patch_size):
        ...
        self.proj1 = nn.Conv2d(in_channels, 32, kernel_size=(7, 7), stride=(2, 2), padding=(2, 2))
        self.proj2 = nn.BatchNorm2d(32)
        self.proj3 = nn.MaxPool2d(3, stride=2, padding=1)
        self.proj4 = nn.Conv2d(32, out_channels, kernel_size=(4, 4), stride=(4, 4))
